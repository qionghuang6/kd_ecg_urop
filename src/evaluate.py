...
def load_classifier_outputs(output_files, classes):
    ...
    for i in range(num_recordings):
        ...
        recording_binary_outputs = [
            1 if entry in ("1", "True", "true", "T", "t") else 0
...
def compute_tier_aucs(labels, outputs):
    # labels and outputs are [records x 3]
    tiers = []
    for tier in range(0, 3):
        auroc = AUROC(task="binary")
        probs = torch.tensor(outputs[:, tier])
        labels_vector = torch.tensor(labels[:, tier].astype(int))
        auc = float(auroc(probs, labels_vector))
        tiers.append(auc)
        print(f"Tier{tier + 1} AUROC: {auc}")
    return tiers
...
def find_best_threshold(weights, labels, outputs, classes, normal_class, idx):
    ...
    best_score = compute_challenge_metric(
        weights, labels, outputs_temp, classes, normal_class
    )
    best_threshold = 0.5
